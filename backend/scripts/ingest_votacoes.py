...
def get_votacoes_periodo(data_inicio, data_fim):
    ...
    while url:
        try:
            ...
            logging.error(f"Erro na lista: {e}")
            break
    return votacoes
...
def process_votacao(conn, vot_res):
    ...
    try:
        r = requests.get(f"{BASE_URL}/votacoes/{base_id}", timeout=20)
        if r.status_code != 200:
            r = requests.get(f"{BASE_URL}/votacoes/{original_id}", timeout=20)
        ...
        with conn.cursor() as cursor:
            ...
            """, (original_id, det['uri'], det['data'], det['dataHoraRegistro'], 
            ...
            for p in det.get('proposicoesAfetadas', []):
                ...
                if pid:
                    ...
                    if cursor.fetchone():
                        cursor.execute("INSERT INTO votacoes_proposicoes (votacao_id, proposicao_id) VALUES (%s, %s) ON CONFLICT DO NOTHING;", (original_id, pid))
                    ...
                    if dep_id:
                        ...
                        if cursor.fetchone():
                            try:
                                ...
                                """, (original_id, dep_id, v.get('tipoVoto'), v.get('dataHoraVoto')))
                                votos_inseridos += 1
                            except Exception as ex_insert:
                                logging.error(f"Erro no INSERT voto {dep_id} em {original_id}: {ex_insert}")
                        else:
                            # Debug: por que o deputado não existe?
                            pass
                if votos_inseridos > 0:
                    logging.info(f"Votação {original_id}: {votos_inseridos} votos inseridos.")
...
def main():
    logging.info("Iniciando ingestão com debug ultra-detalhado...")
    conn = db.get_connect()
    if not conn: return
    try:
        clear_votos_and_props(conn)
        periodos = [("-01-01", "-03-31"), ("-04-01", "-06-30"), ("-07-01", "-09-30"), ("-10-01", "-12-31")]
        for ano in range(2007, 2026):
            logging.info(f"=== Ano: {ano} ===")
            for p_inicio, p_fim in periodos:
                inicio, fim = f"{ano}{p_inicio}", f"{ano}{p_fim}"
                if datetime.strptime(inicio, "%Y-%m-%d").date() > date.today(): continue
                lista = get_votacoes_periodo(inicio, fim)
                logging.info(f"Ano {ano}, Período {inicio} a {fim}: {len(lista)} votações.")
                for i, v in enumerate(lista):
                    process_votacao(conn, v)
                    if (i + 1) % 50 == 0: logging.info(f"Ano {ano}, Progresso {inicio}: {i+1}/{len(lista)}")
    finally:
        conn.close()
