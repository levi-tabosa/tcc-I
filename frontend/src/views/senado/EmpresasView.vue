            ...
            <div class="flex items-start gap-4">
              ...
              <div>
                ...
                <p class="mt-2 text-muted-foreground max-w-2xl text-lg leading-relaxed">
                  Quem são os maiores fornecedores do Senado Federal? Monitore quais empresas recebem recursos da Cota Parlamentar (CEAPS) e fiscalize detalhadamente o destino dessas verbas.
                </p>
              </div>
            </div>

            <!-- Legislatura Selector -->
            <div class="flex items-center gap-3 bg-white/50 backdrop-blur-sm px-5 py-2.5 rounded-2xl border border-purple-500/20 shadow-sm self-start md:self-center">
              <div class="flex flex-col">
                <span class="text-[10px] font-bold text-purple-600 uppercase tracking-widest opacity-70">Filtrar por</span>
                <div class="flex items-center gap-2">
                  <select
                    :value="store.legislatura"
                    @change="store.setLegislatura(Number(($event.target as HTMLSelectElement).value))"
                    class="text-sm font-bold text-purple-700 bg-transparent border-none p-0 focus:ring-0 cursor-pointer min-w-[140px]"
                  >
...
const fetchStats = async () => {
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/api/senado/empresas/estatisticas${legParam}`)
