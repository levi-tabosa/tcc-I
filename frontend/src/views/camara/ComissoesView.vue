    ...
    <main class="flex-1">
      ...
      <section class="relative overflow-hidden pt-16 pb-20 lg:pt-24 lg:pb-32">
        <div class="absolute inset-0 -z-10 overflow-hidden">
          <div class="absolute -top-[10%] -left-[10%] w-[40%] h-[40%] rounded-full bg-primary/20 blur-[120px] animate-pulse" />
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div class="flex flex-col items-center text-center">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-bold uppercase tracking-widest mb-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <h1 class="text-4xl font-extrabold tracking-tight text-foreground sm:text-6xl mb-6 animate-in fade-in slide-in-from-bottom-6 duration-1000">
              Comissões <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">Parlamentares</span>
            </h1>
            
            <p class="max-w-2xl text-lg text-muted-foreground mb-10 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200">
              Acompanhe o coração do processo legislativo. Analise a composição, lideranças e atividades das comissões permanentes e temporárias da Câmara.
            </p>
          </div>
        </div>
      </section>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div class="grid gap-6 sm:grid-cols-3">
            <div 
              ...
              :key="idx"
              class="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-8 transition-all hover:bg-white/10 hover:shadow-2xl hover:shadow-primary/5 active:scale-[0.98] backdrop-blur-md animate-in fade-in zoom-in duration-700"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
              ...
              <div class="lg:col-span-2 relative group">
                <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                  <Search class="h-5 w-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
                </div>
                <input
                  v-model="searchQuery"
                  type="text"
                  placeholder="Nome ou sigla da comissão..."
                  class="block w-full pl-14 pr-6 py-4 bg-background border-2 border-border/50 rounded-2xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all text-base font-medium shadow-sm"
                />
              </div>
              ...
              <div class="relative group">
                <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                  <CalendarRange class="h-5 w-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
                </div>
                <select
                  v-model="filterLegislatura"
                  class="block w-full pl-14 pr-12 py-4 bg-background border-2 border-border/50 rounded-2xl text-foreground font-bold focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all appearance-none cursor-pointer shadow-sm"
                >
          ...
          <div class="mt-12">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div
                ...
                :key="comissao.id"
                class="group relative flex flex-col bg-card border border-border/50 rounded-[40px] shadow-sm hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 overflow-hidden animate-in fade-in slide-in-from-bottom-8"
                :style="{ 'animation-delay': `${(idx % 9) * 100}ms` }"
              >
                <!-- Card Header -->
                <div class="p-8 pb-4">
                  <div class="flex justify-between items-start mb-6">
                    <div class="h-16 w-16 flex items-center justify-center rounded-[24px] bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 group-hover:scale-110 transition-transform duration-500">
                      <span class="text-sm font-black text-primary">{{ comissao.sigla }}</span>
                    </div>
                    <div class="flex flex-col items-end gap-2">
                       <span
                        ...
                        :class="[
                          ...
                          comissao.tipo.toLowerCase().includes('permanente') ? 'bg-primary/5 text-primary border-primary/20' :
                      ...
                      <a
                        ...
                        class="p-2 rounded-xl bg-muted hover:bg-primary hover:text-white transition-all"
                  ...
                  <h3 class="text-xl font-bold text-foreground leading-tight line-clamp-2 min-h-[3.5rem] group-hover:text-primary transition-colors">
                    {{ comissao.nome }}
                  </h3>
                </div>
                ...
                >
                  <div v-if="selectedComissao?.id === comissao.id" class="px-8 pb-8 animate-in fade-in duration-500 overflow-y-auto max-h-[400px] scrollbar-hide">
                    <div class="space-y-4">
                      ...
                      >
                        <div class="h-12 w-12 rounded-2xl overflow-hidden bg-background border-2 border-border shadow-sm group-hover/member:border-primary transition-colors">
                          <img
                        ...
                        <div class="flex-1 min-w-0">
                          <p class="text-sm font-bold text-foreground truncate group-hover/member:text-primary transition-colors">{{ membro.nome }}</p>
                          <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{{ membro.partido }} · {{ membro.uf }}</p>
                        </div>
                        <div v-if="membro.cargo" class="px-2 py-1 rounded-lg bg-primary/10 text-[10px] font-black text-primary border border-primary/20 uppercase whitespace-nowrap">
                          {{ membro.cargo }}
                        </div>
                      </div>
                    </div>
            ...
            <div v-if="comissoesFiltradas.length > displayedCount" class="flex justify-center mt-20 pb-20">
              <button
                @click="loadMore"
                class="group relative px-10 py-5 bg-background border-2 border-primary/30 text-foreground font-black rounded-3xl hover:bg-primary hover:text-white hover:border-primary transition-all duration-300 shadow-xl shadow-primary/5 active:scale-95 overflow-hidden"
              >
