<template>
  <div>
    ...
    <p class="text-sm text-muted-foreground mb-4">
      {{ store.totalProposicoes }} proposições encontradas
    </p>

    <!-- Loading State -->
    <div v-if="store.loading && store.proposicoesList.length === 0" class="text-center py-12">
    ...
    <!-- Empty state -->
    <div v-if="!store.loading && !store.error && store.proposicoesList.length === 0" class="text-center py-12">
      <FileText class="h-12 w-12 text-muted-foreground mx-auto mb-4" />
      <p class="text-muted-foreground">Nenhuma proposição encontrada com os filtros selecionados.</p>
    </div>

    <!-- Load more -->
    <div v-if="store.hasMore && store.proposicoesList.length > 0" class="flex justify-center mt-8">
      <button
      ...
      >
        <span v-if="store.loading">Carregando...</span>
        <span v-else>Carregar mais</span>
        <ChevronDown v-if="!store.loading" class="h-4 w-4" />
      </button>
    </div>
  </div>
</template>
